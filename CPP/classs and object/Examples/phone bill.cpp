...
class Telephone
{
	...
	void calBill()
	{
		...
		if(cust.getCalls() > 0 && cust.getCalls() <= 100)
		{
			bill = 200;
		}
		else if(cust.getCalls() >100 && cust.getCalls() < 150)
		{
			call = cust.getCalls() - 100;
			bill = 200 + (call * 0.60);
		}
		
		else if(cust.getCalls() > 150 && cust.getCalls() <= 200)
		{
			call = cust.getCalls() - 150;
			bill = 200 + 0.60 + (call * 0.50);
		}
		
		else if(cust.getCalls() > 200)
		{
			call = cust.getCalls() - 200;
